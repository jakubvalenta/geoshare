...
fun getUrlQueryParams(query: String?, uriQuote: UriQuote): Map<String, String> = if (query.isNullOrEmpty()) {
    emptyMap()
} else {
...
private fun formatUrlQueryParams(params: Map<String, String?>, uriQuote: UriQuote): String =
    params.filter { it.value != null }.map { "${it.key}=${uriQuote.encode(it.value!!.replace('+', ' '))}" }
        .joinToString("&")
...
fun ensureHttpsScheme(uriString: String): String =
    uriString.replace("^([a-z]+:)?(//)?(.)".toRegex(), "https://$3")
