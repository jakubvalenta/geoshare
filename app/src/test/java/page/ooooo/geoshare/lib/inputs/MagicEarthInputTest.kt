...
class MagicEarthInputTest : BaseInputTest() {
    ...
    fun parseUri_place() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(Position(q = "Central Park")),
    ...
    fun parseUri_search() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(Position(q = "Paris", z = 5.0)),
    ...
    fun parseUri_destinationAddress() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(Position(q = "CH1 6BJ United Kingdom")),
    ...
    fun parseUri_parameterDestinationAddressTakesPrecedenceOverQ() = runTest {
        assertEquals(
            @Suppress("SpellCheckingInspection") ParseUriResult.Succeeded(Position(q = "Reuterplatz 3, 12047 Berlin, Germany")),
    ...
    fun parseUri_parameterNameTakesPrecedenceOverQ() = runTest {
        assertEquals(
            @Suppress("SpellCheckingInspection") ParseUriResult.Succeeded(Position(q = "Reuterplatz")),
