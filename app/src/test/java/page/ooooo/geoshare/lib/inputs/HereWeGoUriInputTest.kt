...
class HereWeGoUriInputTest : InputTest {
    ...
    fun parse_unknownPathOrParams() = runTest {
        assertEquals(ParseResult.Success(), input.parse("https://wego.here.com"))
        assertEquals(ParseResult.Success(), input.parse("https://wego.here.com/"))
        assertEquals(ParseResult.Success(), input.parse("https://wego.here.com/?spam=1"))
    }
    ...
    fun parse_coordinatesQueryParam() = runTest {
        assertEquals(
            ParseResult.Success(persistentListOf(WGS84Point(50.21972, -0.68453, z = 6.93, source = Source.MAP_CENTER))),
    ...
    fun parse_shortLink() = runTest {
        assertEquals(
            ParseResult.Success(persistentListOf(WGS84Point(-38.14749, 145.14347, source = Source.HASH))),
