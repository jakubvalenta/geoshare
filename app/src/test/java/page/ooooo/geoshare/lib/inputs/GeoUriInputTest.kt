...
class GeoUriInputTest : BaseInputTest() {
    ...
    fun parseUri_coordsAndQAndZ() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(WGS84Point(50.123456, -11.123456, q = "foo bar", z = 3.4)),
    ...
    fun parseUri_coordsAndQCoords() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(WGS84Point(50.123456, -11.123456)),
    ...
    fun parseUri_coordsAndQCoordsDiffer_qCoordsTakePrecedence() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(WGS84Point(40.7127400, -74.0059965)),
    ...
    fun parseUri_coordsAndName() = runTest {
        assertEquals(
            @Suppress("SpellCheckingInspection")
            ParseUriResult.Succeeded(WGS84Point(40.7127400, -74.0059965, z = 9.0, name = "Nova Iorque")),
            @Suppress("SpellCheckingInspection")
    ...
    fun parseUri_coordsAndNameInSeparateQueryParam() = runTest {
        assertEquals(
            @Suppress("SpellCheckingInspection")
            ParseUriResult.Succeeded(WGS84Point(40.7127400, -74.0059965, name = "Nova Iorque")),
            @Suppress("SpellCheckingInspection")
    ...
    fun parseUri_coordsInQWithSpace() = runTest {
        assertEquals(
            ParseUriResult.Succeeded(WGS84Point(45.4786785, 9.2473799)),
