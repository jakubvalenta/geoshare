...
import page.ooooo.geoshare.lib.decodeOpenStreetMapGeoHash
import page.ooooo.geoshare.lib.decodeOrganicMapsGeoHash
...
class GeoHashTest {
    ...
    fun decodeWazeGeoHash_returnsZoomBasedOnHashLength() {
        listOf(0, 0, 1, 3, 6, 8, 11, 13, 16, 18).forEachIndexed { i, expectedZoom ->
            assertEquals(expectedZoom, decodeWazeGeoHash("9".repeat(i + 1)).third)
        }
    }
    ...
    fun decodeOpenStreetMapGeoHash_returnsZoomBasedOnHashLength() {
        listOf(0, 0, 1, 4, 7, 10, 13, 16, 19, 22).forEachIndexed { i, expectedZoom ->
            assertEquals(expectedZoom, decodeOpenStreetMapGeoHash("9".repeat(i + 1)).third)
        }
    }
